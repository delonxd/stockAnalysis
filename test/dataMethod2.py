...
class DataAnalysis:
    ...
    def add_eq_data(self, code):
        ...
        df = df_src.reindex(['id_002_rate', 'id_012_dilution_rate'], axis="columns")
    ...
    def add_mir_data(self):
        ...
        path = "..\\basicData\\nationalDebt\\mir_y10_us_akshare.txt"
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            res = json.loads(f.read())

        s0 = self.regular_series('mir_y10_us', pd.Series(res))
        df = pd.DataFrame(s0)
        self.add_df(df)

    def add_crypto_data(self):
        path = "..\\basicData\\crypto\\crypto_btc_sz.txt"
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            res = json.loads(f.read())

        s0 = self.regular_series('cpt_btc', pd.Series(res))
        df = pd.DataFrame(s0)
        self.add_df(df)
    ...
    @if_column_exist
    def get_column(self, df, column, dropna=True):
        if column == 'dt_fs':
        ...
        #     return self.regular_series(column, s1 - s2)
            ...
            s1 = self.smooth_data(column, 'id_102_bs_ll')  # 租赁负债
        ...
        #     return self.regular_series(column, s2 + s3)
            ...
            d0 = {
                'id_261_cfs_ncffoa': '经营活动产生的现金流量净额',
                'id_186_ps_oic': '加： 其他收益',
                'id_197_ps_ooe': '其他业务成本',
                'id_203_ps_noi': '加： 营业外收入',
                'id_205_ps_noe': '减： 营业外支出',
                'id_308_cfs_fe': '财务费用',
                'id_310_cfs_didita': '递延所得税资产减少',
                'id_311_cfs_iiditl': '递延所得税负债增加',
                'id_312_cfs_dii': '存货的减少',
                'id_313_cfs_dior': '经营性应收项目的减少',
                'id_314_cfs_iiop': '经营性应付项目的增加',
                'id_315_cfs_o': '其他',
            }
            s = list()
            for index in d0.keys():
            ...
            s4 = self.get_column(df, 's_086_rental_expense')

            s1 = s[0] - (s[1] - s[2] + s[3] - s[4])
            s2 = (s[5] + s[6] + s[7] + s[8] + s[9] + s[10] + s[11])

            s2 = s2.replace(0, np.nan)
            s3 = (s1 - s2 - s4).dropna()
            ttm = self.get_ttm(get_month_delta(s3, column), 4)
            ...
            s1 = self.get_column(df, 'dv_001_dividend_value')
            s2 = self.get_column(df, 's_018_profit_parent')
            s3 = s1 / s2
            s3[s3 > 2] = 2
            s3[s3 < 0] = 0
            ...
            s1 = self.get_column(df, 's_075_capital_dividend_ratio')
            s1[s1 > 1] = 1
            s1 = (1 - s1)/5
            return self.regular_series(column, s1)
            ...
            s1 = self.get_column(df, 'dv_001_dividend_value')
            s2 = self.get_column(df, 's_063_profit_salary2')
            ...
            s1 = df1['id_281_cfs_crfai'].copy().fillna(0)
            ttm = self.get_ttm(get_month_delta(s1, column), 4)
            s1 = ttm / 0.1
            return self.regular_series(column, s1)

        elif column == 's_077_financing_cash_outflows':
            df1 = df.dropna(subset='s_026_liquidation_asset').copy()
            # s1 = df1['id_288_cfs_cpfdapdoi'].copy().fillna(0)
            # s2 = df1['id_308_cfs_fe'].copy().fillna(0)
            # s3 = (s1 - s2) / 0.1
            # s4 = self.get_ttm(get_month_delta(s3, column), 4)
            s1 = df1['dv_001_dividend_value'].copy().fillna(0)
            s1 = s1 / 0.1
            return self.regular_series(column, s1)

        elif column == 's_078_total_cash_inflows':
            s1 = self.get_column(df, 's_076_financing_cash_inflows')
            s2 = self.get_column(df, 's_063_profit_salary2')
            s3 = s1 + s2
            return self.regular_series(column, s3)

        elif column == 's_079_capital_retention_ratio':
            s1 = self.get_column(df, 's_080_capital_retention')
            s2 = self.get_column(df, 's_063_profit_salary2')
            s2[s2 < 0] = np.nan
            s3 = s1 / s2
            s3[s3 < 0] = 0
            ...
            s1 = self.get_column(df, 's_078_total_cash_inflows')
            s2 = self.get_column(df, 's_077_financing_cash_outflows')
            s3 = s1 - s2
            return self.regular_series(column, s3)

        elif column == 's_081_total_retention_ratio':
            s1 = self.get_column(df, 's_080_capital_retention').cumsum()
            s2 = self.get_column(df, 's_063_profit_salary2').cumsum()
            s2[s2 < 0] = np.nan
            s3 = s1 / s2
            s3[s3 < 0] = 0
            s3 = s3.fillna(np.inf)
            ...
            df1 = df.dropna(subset='id_298_cfs_np').copy()

            s1 = df1['id_054_bs_roua'].copy().fillna(0)  # 使用权资产
            s1 = (s1 - s1.shift(1)).fillna(0)
            s2 = df1['id_102_bs_ll'].copy().fillna(0)  # 租赁负债
            s2 = (s2 - s2.shift(1)).fillna(0)

            s3 = df1['id_316_cfs_coditc'].copy().fillna(0)  # 使用权资产摊销
            s4 = s1 + s3 - s2

            s4 = get_month_data(s4, 's4')
            return self.regular_series(column, s4)
