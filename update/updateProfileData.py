from request.requestProfileData import request_security_profile_cn
from request.requestProfileData import request_company_profile_cn
...
def update_profile_combine():
    ...
    df1 = df1.drop(['first_update', 'last_update', 'stockCode'], axis=1)
    ...
    fs_type_dict = load_json_txt(path)
    s0 = pd.Series(fs_type_dict)
    df['fs_type'] = s0
    ...
    for index, row in ret.iterrows():
        ...
        related_codes = ret[ret['company_name'] == company].index.tolist()
        if len(related_codes) > 1:
            related_codes.pop(related_codes.index(index))
            tmp_list.append(','.join(related_codes))
        else:
            tmp_list.append(None)
    ret['related_stocks'] = tmp_list
